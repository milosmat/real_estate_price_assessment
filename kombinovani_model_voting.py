...
# Provera da li postoji sačuvan model
if os.path.exists('voting_regressor.pkl'):
    voting_regressor = joblib.load('voting_regressor.pkl')
    print("Model loaded from file.")
else:
    # Kreiranje Voting Regressor-a sa kombinovanim modelom i linearnom regresijom
    voting_regressor = VotingRegressor([
        ('xgb', xgb_model),
        ('resnet', combined_model),
        ('linear', LinearRegression())
    ])

    # Treniranje Voting Regressor-a
    voting_regressor.fit(X_train, y_train)
    # Sačuvaj model nakon treniranja
    joblib.dump(voting_regressor, 'voting_regressor.pkl')
    print("Model trained and saved to file.")

# Predikcije sa Voting Regressor-om
predictions_voting = voting_regressor.predict(X_test)

# Vizualizacija predikcija
plt.figure(figsize=(10, 6))
plt.scatter(y_test, predictions_voting, alpha=0.3)
plt.plot([min(y_test), max(y_test)], [min(y_test), max(y_test)], 'r--', lw=2)
plt.xlabel('Actual Prices (EUR)')
plt.ylabel('Predicted Prices (EUR)')
plt.title('Actual vs Predicted Prices with Voting Regressor')
plt.show()

# Izračunavanje grešaka
mae_voting = np.mean(np.abs(y_test - predictions_voting))
mse_voting = np.mean((y_test - predictions_voting) ** 2)
print(f'MAE with Voting Regressor: {mae_voting}')
print(f'MSE with Voting Regressor: {mse_voting}')
